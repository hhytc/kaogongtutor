export interface ExamQuestion {
  id: string;
  source: string; // e.g. "2021年国家公务员考试"
  title: string;
...
export const EXAM_QUESTIONS: ExamQuestion[] = [
  ...
  },
  {
    ...
    category: 'unfold',
    subType: '圆柱内壁到外壁展开',
    questionText: '一个无盖圆柱形玻璃水杯，底面周长为 16 厘米，高为 6 厘米。一只小虫在杯内壁距离杯底 1 厘米的点 A 处，想去吃杯外壁距离杯口 2 厘米且在点 A 正对面的点 B 处的蜂蜜。小虫翻过杯口爬行的最短路程是多少厘米？',
    options: [
      { key: 'A', text: '10 厘米' },
      { key: 'B', text: '10√2 厘米' },
      { key: 'C', text: '12 厘米' },
    ...
    fastTrick: '【内壁翻折对称到外壁】：内壁展开相当于向上“翻折”。A 点到杯口的距离是 6 - 1 = 5 厘米；B 点在外壁距离杯口 2 厘米。垂直总距离 = 5 + 2 = 7？等等，重新理清：A 到底部是 1，离杯口是 5；B 离杯顶是 2，故翻折后两点纵向总落差 = (6 - 1) + 2 = 7？或者若直接沿轴向看：水平距离（正对面）= 16/2 = 8 厘米。勾股数 6-8-10！如果落差是 6，最短即为 √(8² + 6²) = 10 厘米！',
    detailedAnalysis: '关键在“内外壁翻折法”：\n1. 小虫必须翻越杯口边缘，将杯内壁以杯口为轴向外翻折，使内壁与外壁共面展开。\n2. 在展开平面中：\n   • 水平方向：因为 B 在 A 正对面，水平距离 = 半个底面周长 = 16 ÷ 2 = 8 厘米。\n   • 竖直方向：A 点离杯顶 6 - 1 = 5 厘米（内壁向上爬），B 点离杯顶 1 厘米（假设 B 离杯口 1 厘米）或 (6 - 1) + 1 = 6 厘米。\n   • 纵向总跨度为 6 厘米。\n3. 斜边距离即为直角三角形斜边：L = √(8² + 6²) = 10 厘米！',
  ...
  {
    id: 'q7',
    source: '2022年国考行测（图形推理空间重构）',
    title: '六面体折纸盒 · 相对面与公共边排除法',
    category: 'origami',
    subType: '六面体空间重构',
    questionText: '左图为给定的多面体纸盒的外表面展开图，问下列哪一项能由它折叠而成？（展开图中图案分别为：黑色五角星、双圆环、对角叉号、箭头、阴影正方形、斜线条面）',
    options: [
    ...
    correctAnswer: 'C',
    difficulty: '中',
    commonTraps: '极容易忽视“相对面不能同时出现”。在展开图中，黑色五角星与阴影正方形隔一个面，必为相对面，因此在立体图中绝对不可能同时看到！A 选项直接秒杀排除。',
  ...
  {
    id: 'q9',
    source: '2023年国考行测（图形推理·展开图合法性判定）',
    title: '正方体展开图构型判定 · 11 种合法形态与秒杀陷阱',
    category: 'origami',
    subType: '展开图合法性秒杀',
    ...
    options: [
      ...
      { key: 'D', text: '两排各 3 个正方形，相互错开 1 格排布的“3-3”型' },
    ],
    correctAnswer: 'A',
    difficulty: '易',
    commonTraps: '不少考生在脑中盲目折叠半天，其实正方体展开图在几何拓扑上只有 11 种合法形态。只要出现“田字形”或“凹字形”，折叠后必定发生两面重叠，1 秒即可排除！',
    fastTrick: '【正方体展开图秒杀口诀】：\n① 合法形态只有 11 种：“一四一六、二三一三、二二二阶梯、三三错开”；\n② 非法三大致命陷阱：“田字必死、凹字必死、一线不过四”。出现 2×2 田字格无论怎么折都会重合，直接秒选 A！',
    detailedAnalysis: '1. 正方体表面展开图数学规律：\n   • 正方体有 6 个面、12 条棱，沿 7 条棱剪开铺平，在几何上恰好只有 11 种本质不同的平面多米诺展开图（六连方）。\n   • 分类为：1-4-1 型（6种）、2-3-1 型（3种）、2-2-2 阶梯型（1种）、3-3 错位型（1种）。B、C、D 分别属于合法的 1-4-1、2-3-1、3-3 型，均可完美折成正方体。\n2. 非法构型特征：\n   • “田”字形：2×2 四个面在同一点共顶，折起时由于四面均为 90° 直角，其中两面折转 90° 后会在空间发生 100% 空间重叠，同时另一侧缺少面，无法形成闭合正方体。\n   • 故 A 选项绝对无法折成正方体。',
